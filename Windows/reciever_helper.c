...
void recieve_data(SOCKET socket_peer,void *read,uint64_t size){
 		...
 		while(total_needed>0) {												 
	 		...
	 		uint64_t bytes_recieved=recv(socket_peer,(unsigned char *)read+total_recieved,total_needed,0);
...
void download_file(SOCKET socket_peer,void *read,int size,FILE *fo){
		...
		uint64_t total_recieved=0;
 		...
 		while(total_needed>0) {												 
	 		...
	 		uint64_t bytes_recieved = recv(socket_peer,(unsigned char *)read,max_memory,0);
	 		if (bytes_recieved<1) break;

	 		fwrite(read,sizeof(unsigned char),bytes_recieved,fo);
	 		fseek(fo,0L,SEEK_END);	
	 			 			
	 		free(read);
	 		read = (unsigned char *)malloc(max_memory);
...
void recieve_file(SOCKET socket_peer,char filename[100]){
		...
		recieve_data(socket_peer,&expected_size,sizeof(uint64_t));
 		...
 		unsigned char *read=(unsigned char *)malloc(max_memory);
